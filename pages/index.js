...
const DAILY_ASSESSMENTS = [
  ...
  { day: 44, date: 'Apr 12', x: 76, y: 2.8,
    ...
    yNote: 'TLM Assessment Day 44: 2.8/10. Score drops slightly from Day 43 not because the talks failed — that was already the base case — but because the revealed gap between positions is wider than the architecture of the talks suggested. The US demand is apparently full nuclear abandonment including civilian enrichment; Iran\'s demand is permanent Hormuz sovereignty — i.e., the right to charge tolls and control passage. These are not bridgeable in 10 days. What keeps Y above 2.5: the direct diplomatic channel opened in Islamabad is real, Pakistan is still functional as mediator, and Iran\'s public response is "negotiations will continue" rather than "we\'re resuming attacks." Trump\'s blockade threat is an escalation signal but it is not IRGC/SNSC operational movement, which remains the actual watch item. The ceasefire window closing April 22 is the next forcing function.' },
];
