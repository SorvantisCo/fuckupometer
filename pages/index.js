...
const DAILY_ASSESSMENTS = [
  ...
  { day: 94, date: 'Jun 1', x: 74, y: 3.5,
    ...
    yNote: 'TLM Assessment Day 94: 3.5/10. Up 1.5 from Day 80 (2.0) — the single largest upward Y move of the war, and the first that rests on structure rather than mood. For the first time there is a written instrument: a staff-level memorandum of understanding with specific, falsifiable terms (60-day extension, no-toll reopening, a 30-day mine-clearance clock, a uranium-handover framework). That is categorically different from the rhetorical "deals" of March and April, which never produced text. The reason Y is 3.5 and not 5+ is that every load-bearing element is conditional: Trump has not signed, Vance is openly noncommittal, Bessent has stapled the whole thing to Hormuz physically opening first, and the two sides cannot agree publicly on whether the document even requires US withdrawal. Meanwhile the Jun 1 strike exchange shows the operational tempo has not actually de-escalated to match the diplomatic track — the guns are arguing with the paper. Watch items: (1) an actual Trump signature, (2) the first no-toll Western-flagged transit, (3) whether the 30-day mine clock ever starts. If the MOU is signed and a single clean transit occurs, Y moves toward 5. If the Jun 1 exchange becomes a pattern, the paper dies and Y reverts to the 2s.' }
];
